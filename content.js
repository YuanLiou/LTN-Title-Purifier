...
chrome.runtime.sendMessage({ action: 'getApiStatus' }, (response) => {
  if (chrome.runtime.lastError) {
    console.error('[LTN Purify] Error getting API status:', chrome.runtime.lastError.message);
    return;
  }

  if (response && response.isAvailable) {
    // API is available, proceed with the rest of the script
    main();
  } else {
    console.log('[LTN Purify] API is not available. Aborting.');
  }
});
